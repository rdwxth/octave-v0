...
export function SpotifyClone() {
  ...
  useEffect(() => {
    const interval = setInterval(updateCurrentLyric, 500); // Update every half second
  ...
  return (
    <div className="h-screen flex flex-col bg-black text-white overflow-hidden">
      ...
      <div className="md:hidden flex flex-col h-full">
            ...
            <div className="h-full flex flex-col items-center justify-center">
              ...
              <div className="text-center" ref={lyricsRef}>
                {lyrics.map((lyric, index) => (
                  <p
                    key={index}
                    className={`text-lg ${index === currentLyricIndex ? 'text-green-500 font-bold' : 'text-gray-400'}`}
                  >
                    {lyric.text}
                  </p>
                ))}
        ...
        <footer className="bg-black p-4 flex justify-around fixed bottom-0 left-0 right-0" style={{ zIndex: 9999 }}>
          <button className="flex flex-col items-center text-gray-400 hover:text-white" onClick={() => { setView('home'); setSearchQuery(''); }}>
            <Home className="w-6 h-6" />
            <span className="text-xs mt-1">Home</span>
          </button>
          <button className="flex flex-col items-center text-gray-400 hover:text-white" onClick={() => setView('search')}>
            <Search className="w-6 h-6" />
            <span className="text-xs mt-1">Search</span>
          </button>
          <button className="flex flex-col items-center text-gray-400 hover:text-white" onClick={() => setShowCreatePlaylist(true)}>
          ...
          >
            ...
            {miniplayerExpanded && (
              <div className="mt-8 h-[calc(100%-8rem)] flex flex-col">
                ...
                <img src={currentTrack.album.cover_medium} alt={currentTrack.title} className="w-64 h-64 mx-auto object-cover rounded-md mb-4" />
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  />
                  <button onClick={(e) => { e.stopPropagation(); toggleLyricsView(); }}>
